...
summarize_matrix <- function(x, na.rm=FALSE) {
  num_lt_0 <- function(a) sum(a<0,na.rm=na.rm)
  num_btw_1_and_5 <- function(a) sum(a>1 & a<5,na.rm=na.rm)
  num_na <- function(a) sum(is.na(a))
  ...
  summary <- data.frame(
    ...
    num_lt_0 = apply(x,1,num_lt_0),
    num_btw_1_and_5 = apply(x,1,num_btw_1_and_5),
    num_na = apply(x,1,num_na)
  )
    return(summary)
}
